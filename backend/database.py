...
def init_db():
    ...
    _ensure_player_certificate_columns()
    _ensure_user_table()
...
def _ensure_player_certificate_columns():
    """
    Ensure boolean tracking columns exist for players' certificates.
    """
    inspector = inspect(engine)
    if "players" not in inspector.get_table_names():
        return
    ...
    with engine.begin() as conn:
        if "certificate_eligible" not in columns:
            conn.execute(text("ALTER TABLE players ADD COLUMN certificate_eligible BOOLEAN DEFAULT 0"))
        if "certificate_sent" not in columns:
            conn.execute(text("ALTER TABLE players ADD COLUMN certificate_sent BOOLEAN DEFAULT 0"))
