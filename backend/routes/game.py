...
async def create_game_session(game_data: GameSessionCreate, db: Session = Depends(get_db)):
    ...
    # Generate QR code and direct link for joining
    base_url = settings.FRONTEND_BASE_URL.rstrip("/")
    join_url = f"{base_url}/join?pin={pin}"
...
@router.get("/{pin}/join-info")
async def get_join_info(pin: str, db: Session = Depends(get_db)):
    """Get QR code and direct link for lobby joining (for host display)"""
    ...
    join_url = f"{base_url}/join?pin={pin}"
    qr_code = generate_qr_code(join_url)
    return {"join_url": join_url, "qr_code": qr_code}
...
def process_and_email_certificates(game_session_id: int, db: Session):
    """Background task to generate and email certificates to eligible players."""
    game_session = db.query(GameSession).filter(GameSession.id == game_session_id).first()
    if not game_session or not game_session.certificate_template_path:
        return
    ...
    for player in players:
        # Check if they provided a roll number (required for email derivation)
        if not player.roll_number:
            continue
        ...
        if not eligibility["eligible"]:
            player.certificate_eligible = False
            continue
            
        player.certificate_eligible = True
        
        # Eligible: Generate PDF and email
        try:
            certificate_pdf = generate_certificate_pdf(str(template_path), player.name)
            
            # Format email: rollnumber@krmu.edu.in
            recipient_email = f"{player.roll_number.strip()}@krmu.edu.in"
            
            safe_name = re.sub(r"[^A-Za-z0-9_-]", "_", player.name.strip()) or "player"
            filename = f"certificate_{game_session.pin}_{safe_name}.pdf"
            
            # Send the email
            is_sent = send_certificate_email(recipient_email, certificate_pdf, filename)
            player.certificate_sent = is_sent
            
        except Exception as exc:
            import logging
            logging.getLogger(__name__).error(f"Error emailing certificate to {player.name}: {exc}")
            player.certificate_sent = False
            
    db.commit()


@router.post("/{pin}/end")
async def end_game(pin: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """End the game and generate emails"""
    ...
    # Trigger certificate emails in the background
    background_tasks.add_task(process_and_email_certificates, game_session.id, db)
...
async def get_game_results(pin: str, db: Session = Depends(get_db)):
    ...
    for player in players:
        ...
        results["players"].append({
            ...
            "certificate_eligible": player.certificate_eligible,
            "certificate_sent": player.certificate_sent
        })
