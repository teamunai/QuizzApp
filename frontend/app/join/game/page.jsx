              ...
              <div className="mt-4">
                ...
                {certificateStatus.game_finished && certificateStatus.template_uploaded && certificateStatus.eligible ? (
                  <div className="flex flex-col items-center gap-2 mt-4">
                    {certificateStatus.certificate_sent ? (
                      <p className="text-sm font-medium text-green-400">
                        ✓ Certificate sent to your KRMU email
                      </p>
                    ) : (
                      <p className="text-sm font-medium text-yellow-400">
                        ⚠️ Could not email certificate automatically
                      </p>
                    )}
                    <button
                      onClick={async () => {
                        try {
                          const res = await fetch(gameAPI.downloadCertificateUrl(pin, certificateStatus.player_id));
                          if (!res.ok) throw new Error('Download failed');
                          const blob = await res.blob();
                          const url = window.URL.createObjectURL(blob);
                          const a = document.createElement('a');
                          a.href = url;
                          a.download = `certificate_${pin}_${playerName.replace(/[^a-zA-Z0-9_-]/g, '_')}.pdf`;
                      ...
                      className="btn-primary inline-block px-5 py-2 text-sm mt-1"
                    >
