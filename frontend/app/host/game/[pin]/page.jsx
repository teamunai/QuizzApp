...
export default function HostGamePage() {
  ...
  const autoProceedScheduledRef = useRef(false);
  ...
  // Reset auto-proceed flag when question changes
  useEffect(() => {
    autoProceedScheduledRef.current = false;
  }, [currentIndex]);

  // Auto-proceed to next question when timer ends (after brief delay to show results)
  useEffect(() => {
    if (timeLeft !== 0 || !currentQuestion || gameEnded || questions.length === 0) return;
    if (autoProceedScheduledRef.current) return;
    autoProceedScheduledRef.current = true;
    ...
    const t = setTimeout(() => {
      if (currentIndex < questions.length - 1) {
        ...
        gameAPI.end(pin).catch(() => {});
        const socket = getSocket();
        socket.emit("end_game", { pin, final_results: [] });
        setGameEnded(true);
        
        // Delay fetching the final results so the background email processing has time to finish
        setTimeout(() => {
          gameAPI.getResults(pin)
            .then((r) => setFinalLeaderboard(r?.data?.players || []))
  ...
  }, [timeLeft, currentQuestion, currentIndex, questions.length, pin, gameEnded]);
  ...
  const handleNextQuestion = async () => {
    ...
    setFinalResultsLoading(true);
    // Delay getting final results so backend certificate generation has time to complete
    setTimeout(async () => {
      try {
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-white/60 border-b border-white/10">
                      ...
                      <th className="py-2 pr-0">Certificate</th>
                    </tr>
                  </thead>
                  <tbody>
                    {finalLeaderboard.map((player, idx) => (
                      <tr key={player.id || `${player.name}-${idx}`} className="border-b border-white/5">
                        ...
                        <td className="py-2 pr-0">
                          {player.certificate_sent 
                            ? <span className="text-green-400">Sent</span> 
                            : player.certificate_eligible 
                              ? <span className="text-yellow-400">Error Sending</span>
                              : <span className="text-white/40">Ineligible</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
    ...
    <div className="min-h-screen p-4 md:p-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          >
            {currentIndex < questions.length - 1 ? (timeLeft > 0 ? "Skip to Next" : "Next Question") : "End Game"}
          </Button>
        </div>
      </div>
    </div>
